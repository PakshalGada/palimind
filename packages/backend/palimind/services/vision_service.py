    ...
    if cfg_path.exists():
        try:
            return (
                ...
                .get("ollama_base_url", OLLAMA_BASE_URL)
                .rstrip("/")
            )
        except Exception:
            pass
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if description:
                        ...
                        logger.warning(
                            f"[Palivision Vision] {model_name} returned empty description."
                        )
                else:
                    logger.warning(
                        f"[Palivision Vision] {model_name} returned HTTP {response.status_code}"
                    )
