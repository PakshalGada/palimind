...
from palimind.services.ocr_service import extract_text_from_b64
from palimind.services.vision_service import describe_screenshot

logger = logging.getLogger(__name__)

# This router is registered in packages/backend/palimind/api_server.py
...
async def analyze_screen(req: PalivisionRequest):
    ...
    if _global_cfg_path.exists():
        try:
            _ollama_url = json.loads(_global_cfg_path.read_text("utf-8")).get(
    ...
    if ocr_text:
        context_sections.append(f"TEXT VISIBLE ON SCREEN (extracted via OCR):\n{ocr_text}")
    else:
        context_sections.append("TEXT VISIBLE ON SCREEN: (no text detected)")

    if vision_description:
        context_sections.append(f"VISUAL DESCRIPTION OF SCREEN:\n{vision_description}")
    ...
    logger.info(
        f"[Palivision] System prompt built ({len(system_prompt)} chars). Streaming response..."
    )
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=120.0) as client:
                async with client.stream(
                    ...
                    json={"model": req.chat_model, "messages": ollama_messages, "stream": True},
                ) as response:
...
async def warmup_ocr():
    ...
    """
    try:
        from palimind.services.ocr_service import _get_reader
