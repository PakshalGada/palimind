      ...
      {creating ? (
        ...
        <div className="agent-chat-pane">
          <div className="agent-chat-header">
            ...
            <button className="icon-btn" title="Configure" onClick={() => setShowConfig(true)}>
...
function AgentChatInput({
  ...
  return (
    <div className="agent-chat-input">
      ...
      />
      {disabled ? (
        <button className="agent-chat-stop" onClick={() => onStop(agentId)} title="Stop">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
            <rect x="4" y="4" width="16" height="16" rx="2" ry="2" />
          </svg>
        </button>
      ) : (
        <button className="agent-chat-send" onClick={send} disabled={!value.trim()} title="Send">
