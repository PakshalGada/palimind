...
from core.session_store import append_message_to_session, load_sessions, save_sessions
from core.storage.chat_store import ChatVectorStore, search_chat_episodes
from core.memory import get_hierarchical_memory, format_hierarchical_memory_context
            ...
            "sessions": [
                {
                    ...
                    "messages": [
                        {"role": "user", "content": f"Message {i}"} if i % 2 == 0 else {"role": "assistant", "content": f"Response {i}"}
                        for i in range(10)
                    ]
                }
            ]
