...
from core.session_store import load_session_by_id, get_sessions_index_file
...
def format_hierarchical_memory_context(
    ...
    if mid_term_summary and mid_term_summary.strip():
        parts.append(f"### Running Conversation Summary (Mid-term Memory)\n{mid_term_summary.strip()}")
    ...
    if long_term_episodes:
        ...
        if ep_lines:
            parts.append("### Relevant Past Conversations (Long-term Episodic Memory)\n" + "\n".join(ep_lines))
