...
def build_planner_prompt(
    ...
    if route_hints:
        ...
        if route_hints.get("needs_memory"):
            hints.append(
                "- Include an agent with the memory_search tool if past conversations "
                "would help."
            )
        if route_hints.get("needs_code"):
            hints.append(
                "- Include a coding agent with run_python and write_file tools."
            )
        if hints:
...
def normalize_plan(
    ...
    if valid_tools is None:
        from core.llm.mixture_of_expert.tools import get_tool_names
