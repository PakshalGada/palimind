...
from core.document.graph import DocGraph, load_doc_graph
from core.document.tools import DocumentToolSet
from core.exceptions import EmbeddingError, ResponseError
from core.generative.responder import generate_response_stream
from core.storage.db import fts_search, get_connection, get_file_summary
...
class DocumentEngine:
    ...
    def __init__(
        ...
        self.ollama_url = ollama_url or config.get(
        ...
        self.light_model = (
            light_model or config.get("light_model", "") or self.chat_model
        )
    ...
    def _index_exists(self) -> bool:
        from core.storage.vector_store import _index_path as _vec_index_path
    ...
    def _rrf_fuse(ranked_lists: list[list[dict]], k: int = 60) -> list[dict]:
        ...
        for ranked in ranked_lists:
            for rank, r in enumerate(ranked):
                ...
                entry = fused.setdefault(
                    key, {"result": r, "score": 0.0, "types": set()}
                )
        ...
        for entry in fused.values():
            ...
            entry["result"]["search_type"] = "+".join(
                t for t in ("semantic", "keyword") if t in entry["types"]
            ) or "graph"
    ...
    def _expand_window(
        ...
        from core.storage.db import get_chunk_neighbors
    ...
    def _retrieve_context_impl(
        ...
        if not index_ok:
            errors.append(
                "No vector index found — run 'pm add' to index files first"
            )
        ...
        if index_ok and base_variants:
            try:
                ...
                variant_vectors = {
                    qv: vec for qv, vec in zip(base_variants, vecs) if vec
                }
        ...
        if base_variants:
            ...
            for qv in base_variants[1:]:
                pending.append(
                    ("keyword", executor.submit(_keyword, qv, max(limit // 2, 1)))
                )
        ...
        fresh_rewrites = [
            rv
            for rv in rewritten_variants
            if rv.lower() not in existing_lower
        ]
        if index_ok and fresh_rewrites:
            ...
            try:
                ...
                rw_pending = []
                for qv, vec in zip(fresh_rewrites, rvecs):
                    if vec:
                        rw_pending.append(executor.submit(_semantic, qv, vec))
                for qv in fresh_rewrites:
                    rw_pending.append(
                        executor.submit(_keyword, qv, max(limit // 2, 1))
                    )
        ...
        if files_filter:
            all_results = [
                ...
                if any(f in r.get("file_path", "") for f in files_filter)
            ]
        ...
        for r in selected_results:
            ...
            if start_ts is not None:
                end_part = (
                    f"–{self._fmt_ts(float(end_ts))}" if end_ts is not None else ""
                )
        ...
        has_media = any(
            r.get("media_start_ts") is not None for r in selected_results
        )
