...
from core.config import load_config, palimind_dir
...
class DocGraph:
    ...
    def add_file_node(self, file_path: str, label: str = "", doc_type: str = "other", doc_year: int | None = None, summary: str = "") -> str:
...
def build_doc_graph(root: Path, ollama_url: str, light_model: str = "") -> DocGraph:
    ...
    conn = get_connection(root)
    try:
        from core.storage.db import get_rich_chunks_for_file
    ...
    try:
        ...
        for i in range(0, len(file_excerpts), batch_size):
            ...
            batch_text = "\n---\n".join(
                f"FILE {idx+1}: {fp}\n{excerpt}"
                for idx, (fp, excerpt) in enumerate(batch)
            )
            batch_prompt = (
                ...
                if target_node and source_node:
                    if target_node["type"] == "entity" and source_node["type"] == "file":
                        ...
                        for other_edge in g.edges:
                            if other_edge["relation"] == "references" and other_edge["target"] == entity_nid:
                                other_file_nid = other_edge["source"]
                                if other_file_nid != edge["source"]:
                                    existing = any(
                                        e["source"] == edge["source"] and e["target"] == other_file_nid
                                        for e in g.edges
                                    )
                                    if not existing:
                                        g.add_edge(edge["source"], other_file_nid, "related_via", weight=0.5)
    except Exception as exc:
...
def load_doc_graph(root: Path, ollama_url: str = "", light_model: str = "", force_rebuild: bool = False) -> DocGraph:
