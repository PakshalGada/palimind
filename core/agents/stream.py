...
from core.agents.runtime import run_with_definition
...
async def agent_event_stream(
    ...
    task = asyncio.create_task(
        run_with_definition(defn, input, session_id, emit=emit_event)
    )
    try:
        while True:
            if task.done():
        ...
        try:
            ...
            yield {"type": "agent:completed", "output": "[Agent run cancelled]", "status": "cancelled"}
