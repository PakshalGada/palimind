...
from core.agents.runtime import is_running, run_with_definition
from core.settings import AGENT_SCHEDULER_TICK
...
def ensure_results_session(field_root: Path) -> str:
    from core.session_store import add_new_session, load_sessions
...
async def post_result(field_root: Path, defn_name: str, input: str, output: str, status: str) -> None:
    """Post a run's result into the field's _agent_results session."""
    from core.session_store import append_message_to_session
...
def check_watcher_triggers(root: Path, changed_path: str = "") -> None:
    ...
    for defn in get_registry().enabled_agents():
        ...
        if is_running(defn.id):
            continue
        input = f"[Watcher trigger] File changed: {rel}\nInvestigate the change and act accordingly."
        asyncio.run_coroutine_threadsafe(
            _fire_agent(defn, input, source="watcher"), _loop
        )
