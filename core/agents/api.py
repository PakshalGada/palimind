...
from core.agents.stream import agent_event_stream
from core.llm.mixture_of_expert.tools import TOOL_REGISTRY
...
def _agent_item(defn: AgentDefinition) -> dict[str, Any]:
    from core.agents.runtime import is_running
...
async def validate_cron(req: Request):
    from core.agents.catalog import validate_cron as vc
...
async def run_agent(agent_id: str, req: Request):
    ...
    if defn is None:
        async def err_stream():
            yield f'data: {{"type": "error", "text": "Agent not found"}}\n\n'
